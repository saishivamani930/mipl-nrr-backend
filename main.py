...
from ipl_api.points_table import compute_sorted_table
from ipl_api.cricbuzz_fixtures import fetch_cricbuzz_innings_aggregates, KNOWN_MATCH_IDS
from ipl_api.nrr_math import apply_match_batting_first
        ...
        try:
            ...
            for row in updated:
                new_standings["teams"].append({
                    "team": row.get("team", ""),
                    "code": row.get("team", ""),
                    "matches": row.get("played", row.get("matches", 0)),
                    "won": row.get("won", 0),
                    "lost": row.get("lost", 0),
                    "nr": row.get("nr", 0),
                    "tied": row.get("tied", 0),
